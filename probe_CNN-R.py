                ...
                if train_step % opts.valid_freq == 0:
                    ...
                    if val_acc >= max_val_acc:
                        ...
                        for _, test_batch in test_batches:
                            p_test_batch = [x_test[i, 0] for i in test_batch]
                            s_test_batch = [x_test[i, 1] for i in test_batch]
                            p_test_batch = prepare_data_for_CNN(p_test_batch, opts)
                            s_test_batch = prepare_data_for_CNN(s_test_batch, opts)

                            test_probs_batch = sess.run(prob, feed_dict={input_p: p_test_batch,
                                                                         input_s: s_test_batch,
                                                                         dropout_keep_prob: 1.0})
...
def main(data_path, model_archive_path, log_path, save_path, from_scratch, unfrozen,
